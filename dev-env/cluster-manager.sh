...
declare -A CLUSTERS=(
  ["cluster1"]="ex-esdb-volumes.yaml,ex-esdb-network.yaml,ex-esdb-cluster.yaml,cluster,5 nodes (0-4)"
  ["cluster2"]="ex-esdb-volumes2.yaml,ex-esdb-cluster2.yaml,cluster2,8 nodes (10-17)"
  ["cluster3"]="ex-esdb-volumes3.yaml,ex-esdb-cluster3.yaml,cluster3,2 nodes (20-21)"
...
declare -A PROD_OVERRIDES=(
  ...
  ["cluster3"]="ex-esdb-prod-override.yaml"
)
...
get_cluster_config() {
  ...
  "cluster2")
    ...
    ;;
  "cluster3")
    # Format: volumes3.yaml,cluster3.yaml,project,description
    case $config_type in
    "project") echo "${config[2]}" ;;
    "description") echo "${config[3]}" ;;
    "files") echo "${config[0]},${config[1]}" ;;
...
get_cluster_status() {
  ...
  elif [[ "$project_name" == "cluster3" ]]; then
    # For cluster3 project, containers are named ex-esdb20, ex-esdb21
    filter_pattern="name=^ex-esdb2[0-1]$"
  else
...
start_cluster() {
  ...
  if [[ "$cluster_name" == "cluster1" ]]; then
    ...
    sudo chown "$USER" -R /volume/
  elif [[ "$cluster_name" == "cluster3" ]]; then
    print_color $CYAN "Creating data directories for cluster3..."
    sudo mkdir -p /volume/ex-esdb/data{20..21}
    sudo chown "$USER" -R /volume/
  fi
...
start_cluster_enhanced() {
  ...
  # Start the cluster
  cd "$SCRIPT_DIR"
  if [[ "$cluster_name" == "cluster1" ]] || [[ "$cluster_name" == "cluster2" ]] || [[ "$cluster_name" == "cluster3" ]]; then
    docker-compose $compose_args --profile cluster -p $project up --remove-orphans --build -d
  else
    docker-compose $compose_args -p $project up --remove-orphans --build -d
  fi
...
show_menu() {
  ...
  echo
  print_color $BLUE "  [1] Start Cluster1 (5 nodes)"
  print_color $BLUE "  [2] Start Cluster2 (8 nodes)"
  print_color $BLUE "  [3] Start Cluster3 (2 nodes)"
  print_color $BLUE "  [4] Start Swarm"
  print_color $BLUE "  [a] Start All Clusters"
  echo
  print_color $YELLOW "  [5] Stop Cluster1"
  print_color $YELLOW "  [6] Stop Cluster2"
  print_color $YELLOW "  [7] Stop Cluster3"
  print_color $YELLOW "  [8] Stop Swarm"
  print_color $YELLOW "  [z] Stop All Clusters"
  echo
  print_color $PURPLE "  [r1] Restart Cluster1"
  print_color $PURPLE "  [r2] Restart Cluster2"
  print_color $PURPLE "  [r3] Restart Cluster3"
  print_color $PURPLE "  [r4] Restart Swarm"
  echo
  print_color $CYAN "  [l1] Show Cluster1 Logs"
  print_color $CYAN "  [l2] Show Cluster2 Logs"
  print_color $CYAN "  [l3] Show Cluster3 Logs"
  print_color $CYAN "  [l4] Show Swarm Logs"
  echo
...
main() {
  while true; do
    ...
    3) start_cluster_enhanced "cluster3" ;;
    4) start_cluster "swarm" ;;
    a | A)
      start_cluster_enhanced "cluster1"
      echo
      start_cluster_enhanced "cluster2"
      echo
      start_cluster_enhanced "cluster3"
      ;;
    5) stop_cluster "cluster1" ;;
    6) stop_cluster "cluster2" ;;
    7) stop_cluster "cluster3" ;;
    8) stop_cluster "swarm" ;;
    z | Z)
      for cluster in "${!CLUSTERS[@]}"; do
    ...
    r2 | R2)
      ...
      ;;
    r3 | R3)
      stop_cluster "cluster3"
      echo
      start_cluster_enhanced "cluster3"
      ;;
    r4 | R4) restart_cluster "swarm" ;;
    l1 | L1) show_logs "cluster1" ;;
    l2 | L2) show_logs "cluster2" ;;
    l3 | L3) show_logs "cluster3" ;;
    l4 | L4) show_logs "swarm" ;;
